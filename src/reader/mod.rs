...
#[derive(Clone)]
pub struct IndexReaderBuilder<D = Document> {
...
where D: DocumentAccess
{
    ...
    pub fn try_into(self) -> crate::Result<IndexReader<D>> {
    ...
    #[must_use]
    pub fn reload_policy(mut self, reload_policy: ReloadPolicy) -> IndexReaderBuilder<D> {
    ...
    #[must_use]
    pub fn warmers(mut self, warmers: Vec<Weak<dyn Warmer>>) -> IndexReaderBuilder<D> {
    ...
    /// the [`IndexReader`].
    #[must_use]
    pub fn num_warming_threads(mut self, num_warming_threads: usize) -> IndexReaderBuilder<D> {
        self.num_warming_threads = num_warming_threads;
        self
    }
}

impl<D: DocumentAccess> TryInto<IndexReader<D>> for IndexReaderBuilder<D> {
    type Error = crate::TantivyError;

    fn try_into(self) -> crate::Result<IndexReader<D>> {
        IndexReaderBuilder::try_into(self)
    }
}

struct InnerIndexReader<D> {
    doc_store_cache_num_blocks: usize,
    index: Index<D>,
    warming_state: WarmingState,
    searcher: arc_swap::ArcSwap<SearcherInner<D>>,
    searcher_generation_counter: Arc<AtomicU64>,
    searcher_generation_inventory: Inventory<SearcherGeneration>,
}

impl<D: DocumentAccess> InnerIndexReader<D> {
    fn new(
        doc_store_cache_num_blocks: usize,
        index: Index<D>,
        warming_state: WarmingState,
    ...
    fn open_segment_readers(index: &Index<D>) -> crate::Result<Vec<SegmentReader<D>>> {
    ...
    fn create_searcher(
        ...
        searcher_generation_counter: &Arc<AtomicU64>,
        searcher_generation_inventory: &Inventory<SearcherGeneration>,
    ) -> crate::Result<Arc<SearcherInner<D>>> {
...
#[derive(Clone)]
pub struct IndexReader<D = Document> {
    inner: Arc<InnerIndexReader<D>>,
    _watch_handle_opt: Option<WatchHandle>,
}

impl<D: DocumentAccess> IndexReader<D> {
    #[cfg(test)]
    ...
    pub fn searcher(&self) -> Searcher<D> {
        self.inner.searcher()
    }
}
